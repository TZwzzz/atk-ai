...
#include "my_atk_ssd_object_recognize_track_rtsp.h"
...
void *rkmedia_rknn_thread(void *args)
{
  ...
  int model_len = 0;
  unsigned char *model;
  static char *model_path = "/demo/bin/ssd_inception_v2_rv1109_rv1126.rknn";
  ...
  while (!quit)
  {
    ...
    for (int i = 0; i < detect_result_group.count; i++)
    {
      ...
      printf("detect_result_group.results[i] = %s\r\n",detect_result_group.results[i].name);
      ...
      printf("border=(%d %d %d %d)\nobject=%s\n", left, top, w, h,detect_result_group.results[i].name);
      //调用追踪函数
      trackObject = false;
      t_rect.point = cv::Point(left,top);
      t_rect.w = w;
      t_rect.h = h;
      printf("getObject\r\n");
      if(one_off)//只创建一次
      {
        ...
        pthread_create(&camshift_tidp,NULL,camshift_thread,NULL);
      }
      break;
    }
...
//自适应均值迁移法目标跟踪线程
void *camshift_thread(void *args)
{
  using namespace cv;
  using namespace std;
  ...
  while(true)
  {
    ...
    Mat frame = Mat(video_height, video_width, CV_8UC3, RK_MPI_MB_GetPtr(src_mb));
    //将图像转化成HSV颜色空间
    ...
    {
      //得到目标区域图像
      selection_Cam = Rect(t_rect.point.x,t_rect.point.y,t_rect.w,t_rect.h);
      //目标区域的HSV颜色空间
      Mat roi(hue,selection_Cam);
      //计算直方图和直方图归一化
    ...
    //在图像中绘制寻找到的跟踪窗口
    rectangle(frame,selection_Cam,Scalar(0,255,255),3,LINE_AA);
    putText(frame,track_name[0],Point(selection_Cam.x,selection_Cam.y),FONT_HERSHEY_TRIPLEX, 3, Scalar(0,0,255),4,8,0);
#else
    //自适应均值迁移法跟踪目标
    RotatedRect trackBox = CamShift(backproj,selection_Cam,
        TermCriteria(TermCriteria::EPS|TermCriteria::COUNT,10,1));
    Point2f pts[4];
    trackBox.points(pts);//得到旋转矩阵顶点坐标
    //画框
    for(int i=0;i<4;i++)
		{
			line(frame,pts[i],pts[(i+1)%4],Scalar(0,255,255),5);
		}
    putText(frame, track_name[0], Point(pts[0].x, pts[0].y), FONT_HERSHEY_TRIPLEX, 3, Scalar(0,0,255),4,8,0);
#endif
    //显示结果
    RK_MPI_SYS_SendMediaBuffer(RK_ID_VENC, 0, src_mb);
    RK_MPI_MB_ReleaseBuffer(src_mb);
